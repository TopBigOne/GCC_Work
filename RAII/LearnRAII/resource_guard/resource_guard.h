...
namespace raii {
    ...
    class MemoryGuard {
    public:
        explicit MemoryGuard(std::size_t size) : ptr_(new T[size]()), size_(size) {
            std::cout << "[MemoryGuard] 分配 " << size << " 个 " << typeid(T).name() << "\n";
        }

        MemoryGuard(MemoryGuard &&other) noexcept : ptr_(other.ptr_), size_(other.size_) {
            other.ptr_ = nullptr;
            other.size_ = 0;
        }
