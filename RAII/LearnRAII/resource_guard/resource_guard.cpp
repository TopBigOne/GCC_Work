...
namespace raii {
    ...
    FileGuard::FileGuard(const std::string &path, const char *mode) : file_(std::fopen(path.c_str(), mode)) {
        if (!file_)
            throw std::runtime_error("FileGuard: 无法打开文件: " + path);
    ...
    void demo_resource_guard() {
        ...
        std::cout << "\n[std::unique_ptr 对比]\n";
        {
            auto file_deleter = [](FILE *f) {
                if (f) {
                    std::fclose(f);
                    std::cout << "[unique_ptr] 自动关闭 FILE*\n";
                }
            };
            // 自定义删除器
            auto my_file_deleter = [](FILE*f) {
                if (f) {
                    std::fclose(f);
                    std::cout << "[unique_ptr] 自动关闭 FILE*\n";
                }
            };
            // 啥要用 decltype ？decltype 解决了深问题？
            // decltype(my_file_deleter) 会在编译时推导出 my_file_deleter 的实际类型，并将其作为 std::unique_ptr 的模板参数
            std::unique_ptr<FILE, decltype(file_deleter)> uf(std::fopen("/tmp/raii_unique.txt", "w+"), file_deleter);
            if (uf) {
                std::fputs("unique_ptr file\n", uf.get());
            }
