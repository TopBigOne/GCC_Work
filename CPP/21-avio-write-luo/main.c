...
uint8_t *readFile(char *path, size_t *length) {
    printf("step 1: read file to memory\n");
    FILE    *pfile;
    uint8_t *data;
...
/* 读取 AVPacket 回调函数 */// 0x14d008800
/**
 *
 * @param opaque   avio_alloc_context() 函数 中 &bd 的地址
 *                  &bd 是 readFile() 函数，获取的文件数据
 * @param buf      avio_alloc_context() 函数 中 avio_ctx_buffer 的地址
 * @param buf_size avio_alloc_context() 函数 中  avio_ctx_buffer_size 的大小
 * @return
 */
static int read_packet(void *opaque, uint8_t *buf, int buf_size) {
    av_log(NULL, AV_LOG_FATAL, "read_packet()\n");
    ...
    // copy 数据，将 bd的数据copy到 avio_ctx_buffer 中；
...
int main() {
    ...
    char filename[] = "/Users/dev/Documents/GCC_Work/CPP/21-avio-write-luo/juren-30s.mp4";
    ...
    printf("|---------------------------------------------------------------|\n");
    printf("| &bd      address         is : %p \n", &bd);
    printf("| avio_ctx_buffer  address is : %p \n", avio_ctx_buffer);
    printf("| avio_ctx_buffer_size     is : %d \n", avio_ctx_buffer_size);
    printf("|---------------------------------------------------------------|\n");
    /**
     * read_packet : 定义输入
     * Debug 看 avio_ctx_buffer ，avio_ctx_buffer_size 的 地址；
     */
    avio_ctx = avio_alloc_context(avio_ctx_buffer,
                                  avio_ctx_buffer_size,
                                  0,// 由 自定义的回调函数 向 buffer 填充数据，FFmpeg API 函数取走数据
                                  &bd,// 这是 BufferData 的数据，是readFile() 函数提供的，以便让*opaque  有数据
                                  &read_packet,
                                  NULL,
                                  &seek_in_buffer);
    if (!avio_ctx) {
